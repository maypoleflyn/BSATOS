...
getPvals <-function(Gprime)
    {       
        ...
        gp<-data.frame(Gprime,pval);	
        return(gp);
      
    }


 getFDRThreshold <- function(gp, alpha = 0.01)
{
  #  sortedPvals <- sort(pvalues, decreasing = FALSE);
  #  pAdj <- p.adjust(sortedPvals, method = "BH");
     sortedgp<-gp[order(gp$pval),]; 
     pAdj <- p.adjust(sortedgp$pval,method="BH");
     merged<-data.frame(sortedgp,pAdj);
        
    if (!any(pAdj < alpha)) {
        fdrThreshold <- 0
    } else {
    fdrThreshold <- merged[max(which(merged$pAdj < alpha)),]$Gprime
    }
    return(fdrThreshold);
}
