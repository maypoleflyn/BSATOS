#!/usr/bin/Rscript
 ...
 mem.limits(nsize= 10000,vsize=10000);
...
ke2<- function(POS, Stat, windowSize = 1e6, ...)
{
    if (windowSize <= 0)
...
if(argv[5] ==1){
  ...
  G1M<-ke2(data[,2],data[,11],size);
  G750<-ke2(data[,2],data[,11],size1);
  G500<-ke2(data[,2],data[,11],size2);
  G250<-ke2(data[,2],data[,11],size3);
  data<-data.frame(data,G1M,G750,G500,G250);
